...
async function syncBatch(
  ...
  for (const [type, dbId] of Object.entries(NOTION_DATABASE_IDS)) {
    ...
    for (const page of response.results) {
      await syncPage(notion, sanity, page.id, collectionMap);
      total++;
    }
  }

  console.log(`\nDone. Synced ${total} pages total.`);
}
...
async function main() {
  ...
  const collectionMap = await buildCollectionMap(sanity);
  ...
  if (args.includes("--page-id")) {
    ...
    await syncPage(notion, sanity, pageId, collectionMap);
  } else if (args.includes("--batch")) {
    await syncBatch(notion, sanity, collectionMap);
  } else {
